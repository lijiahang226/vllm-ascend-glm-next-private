...
#
"""Ascend-specific speculator logic shared by Eagle and MTP on the v2 runner.

Ascend attention backends need more information than upstream's (e.g.
``seq_lens_cpu`` from the input batch), so the v2 speculators override the
attention-metadata building and the full-graph parameter updates.  Eagle and
MTP share the same ``AutoRegressiveSpeculator`` base and the same
``load_eagle_model`` draft loader, so all Ascend-specific behavior lives in
:class:`AscendSpeculatorMixin`.
"""
...
def _current_draft_step(speculator) -> int:
    """The current draft step as a Python int.

    Upstream's v2 MTP path never forwards the draft step to the draft model
    (``spec_step_idx`` stays 0), which breaks multi-layer MTP models such as
    GLM-5.  One device-to-host sync per draft step is negligible compared to
    the draft forward pass itself.
    """
    return int(speculator.current_draft_step.item())


class AscendSpeculatorMixin:
    """Ascend-specific behavior for the v2 autoregressive speculators.

    Mix this in front of the upstream speculator class, e.g.::
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        """Override GPU speculator __init__ for Ascend NPUs.
        attention metadata building in Ascend backend needs more information,
    ...
    def propose(
        ...
        """Override GPU speculator.propose for Ascend NPUs,
    ...
    def _run_model(
        ...
        Besides the Ascend seq_lens_cpu refresh, this forwards the current
        draft step to the draft model through ``spec_step_idx``: upstream's
        v2 MTP path leaves it at 0, which would always select the first MTP
        layer on multi-layer MTP models such as GLM-5.
        """
        ...
        with set_forward_context(
            ...
            if self.method == "mtp":
                # GLM-5 MTP selects its layer from spec_step_idx; upstream's
                # v2 MTP path does not forward it.  Eagle draft models have no
                # spec_step_idx kwarg, so only MTP receives it.
                model_inputs["spec_step_idx"] = _current_draft_step(self)
    ...
    def _greedy_sample_draft(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.method == "mtp":
            # Forward the draft step so multi-layer MTP models select the
            # correct layer (see _run_model).
            logits = self.model.compute_logits(
                hidden_states,
                spec_step_idx=_current_draft_step(self),
            )
        else:
    ...
    def sample_draft(
        ...
        """Override DraftModelSpeculator.sample_draft to forward the step."""
...
def build_attn_metadata_wrapper():
    ...
    original_func = vllm.v1.worker.gpu.spec_decode.speculator.build_attn_metadata
    try:
        vllm.v1.worker.gpu.spec_decode.speculator.build_attn_metadata = build_attn_metadata
        yield
    finally:
        vllm.v1.worker.gpu.spec_decode.speculator.build_attn_metadata = original_func
