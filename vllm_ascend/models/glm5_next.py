...
class AscendGlm5NextCompressorStateCache(CompressorStateCache):
    ...
    request's absolute logical position to the physical page owned by the
    allocator. CANN ``key_pool`` requires this state cache to use FP32 with
    shape ``[num_physical_blocks, block_size, 2 * head_dim]`` (plan §5.1).
    """
...
class AscendSparseAttnIndexerKpool(nn.Module):
    ...
    def forward_ascend(
        ...
        *,
        ...
        KeyPool takes over the K projection, gate projection, optional
        LayerNorm, cross-chunk tail state and KPool compression (plan §6);
        PoolKeyIndexer takes over pool-level scoring, Top-K, pool expansion
        and tail token append (plan §7).
        """
        ...
        # state_cache is mutated in place (Tensor(a!) alias); state_metadata
        # carries the persistent NPU buffers built by
        # AscendIndexerKPoolStateMetadataBuilder (plan §5.2). KeyPool has no
        ...
        pooled_key = torch.ops._C_ascend.key_pool(
            ...
            state_metadata.start_pos,
            norm_weight=norm_weight,
            norm_bias=norm_bias,
            cos=None,  # RoPE unsupported
            sin=None,  # RoPE unsupported
            cu_seqlens=state_metadata.cu_seqlens,
            seqused=None,
        ...
        self._scatter_paged_cache(
            indexer_cache,
            indexer_metadata.slot_mapping[:num_tokens].to(torch.int64),
            rows,
        ...
        indices, _ = torch.ops._C_ascend.pool_key_indexer(
            ...
            indexer_metadata.pool_tail_k,
            actual_seq_q=indexer_metadata.actual_seq_q,
...
class AscendGlm5NextIndexer(nn.Module):
    ...
    def forward(
        ...
        Q stays on ``wq_b``; KeyPool owns the K projection (first ``head_dim``
        ...
        if self.rope_dim > 0:
            # KeyPool's current torch interface has no RoPE input (cos/sin are
            # rejected by the binding). Do not silently skip K RoPE: fail fast
            # until a KeyPool RoPE path exists (plan §6).
        ...
        # projection to avoid recomputing K, which KeyPool projects inside.
        weights_weight = self.wk_weights_proj.weight[self.head_dim :]
        weights = torch.nn.functional.linear(hidden_states, weights_weight)
        q = q.to(torch.bfloat16)

        # Upstream folds the FP8 Q scale into weights. Q remains BF16 on
        # Ascend, so only the model-level factor is required here.
        # PoolKeyIndexer applies 1/sqrt(head_dim) and the per-head ReLU
        # internally, so the framework must NOT scale by head_dim again
        # (plan §7).
