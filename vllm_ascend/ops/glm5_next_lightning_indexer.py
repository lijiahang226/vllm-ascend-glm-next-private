...
if HAS_TRITON:
    from vllm_ascend.ops.triton.glm5_next_lightning_indexer import (
        glm5_next_lightning_indexer_triton_scores,
    )
else:
    glm5_next_lightning_indexer_triton_scores = None
...
def _triton_chunked_pool_topk(
    ...
    """Score every pool in parallel Triton chunks, then run one device top-k.

    Sequence length only changes the two-dimensional launch grid and output
    workspace.  Query rows are bounded in small chunks to keep workspace
    usage stable for both decode and large prefills.
    """
    ...
    for query_start in range(0, query.shape[0], INDEXER_KPOOL_QUERY_CHUNK_SIZE):
        query_end = min(
            query_start + INDEXER_KPOOL_QUERY_CHUNK_SIZE,
            query.shape[0],
        )
        chunk_query = query[query_start:query_end]
        chunk_weights = weights[query_start:query_end]
        weighted_query = (
            chunk_query
            * chunk_weights.to(torch.bfloat16).unsqueeze(-1)
        ).sum(dim=1)
        scores = glm5_next_lightning_indexer_triton_scores(
            weighted_query,
            indexer_cache,
            request_ids[query_start:query_end],
            indexer_seq_lens,
            indexer_block_table,
        ...
        output[query_start:query_end] = torch.where(
            valid,
            pool_ids.to(torch.int32),
            torch.full_like(pool_ids, -1, dtype=torch.int32),
        )
    return output
...
def _can_use_triton(
    ...
    if not envs.VLLM_ASCEND_ENABLE_GLM5_NEXT_TRITON_INDEXER:
        return False
    if not HAS_TRITON or glm5_next_lightning_indexer_triton_scores is None:
        return False
