...
template<typename COMP> class KeyPoolBlockCube {
...
private:
    ...
    __aicore__ inline void LoadBToL0(const RunInfo &info, LocalTensor<HIDDEN_STATES_T> bL0Tensor, LocalTensor<HIDDEN_STATES_T> wL1Tensor,
...
template <typename COMP> __aicore__ inline void KeyPoolBlockCube<COMP>::Init(
        __gm__ uint8_t *hidden_states,
        __gm__ uint8_t *wk,
        __gm__ uint8_t *gateWeight,
        __gm__ uint8_t *stateCache,
...
template <typename COMP>
__aicore__ inline void KeyPoolBlockCube<COMP>::InitGlobalBuffers(const GlobalTensor<MM1_OUT_T>& kvMm1ResGm, const GlobalTensor<MM1_OUT_T>& scoreMm1ResGm)
{
...
template <typename COMP>
__aicore__ inline void KeyPoolBlockCube<COMP>::CopyWeightGmToL1(LocalTensor<HIDDEN_STATES_T> wL1Tensor,
    uint32_t hIdx, uint32_t kBase, uint32_t coffId)
{
    ...
    uint32_t wgateL1Offset = constInfo_.dBaseSize * BlockElementNum<HIDDEN_STATES_T>(); // wgate与wkv的起始点相隔dBaseSize个32B
...
template <typename COMP>
__aicore__ inline void KeyPoolBlockCube<COMP>::LoadBToL0(const RunInfo &info, LocalTensor<HIDDEN_STATES_T> bL0Tensor, LocalTensor<HIDDEN_STATES_T> wL1Tensor,
    uint32_t kStart, uint32_t kBase, uint32_t nStart, uint32_t nDealSize)
{
...
template <typename COMP>
__aicore__ inline void KeyPoolBlockCube<COMP>::MatrixMmad(LocalTensor<T> cL0Tensor, LocalTensor<HIDDEN_STATES_T> aL0Tensor,
    LocalTensor<HIDDEN_STATES_T> bL0Tensor, uint32_t mActSize, uint32_t nDealSize, uint32_t kActSize, bool isInitL0C)
{
                ...
                for (uint32_t nL0 = 0; nL0 < nDealSize; nL0 += N_L0_BASE) {
                    ...
                    for (uint32_t kL0 = 0; kL0 < kSize; kL0 += K_L0_BASE) {
                        ...
                        LocalTensor<HIDDEN_STATES_T> aL0Tensor =
                            tmpBufL0A.GetWithOffset<HIDDEN_STATES_T>(L0A_PP_SIZE / sizeof(HIDDEN_STATES_T), l0abBufId * L0A_PP_SIZE);
                        LocalTensor<HIDDEN_STATES_T> bL0Tensor =
                            tmpBufL0B.GetWithOffset<HIDDEN_STATES_T>(L0B_PP_SIZE / sizeof(HIDDEN_STATES_T), l0abBufId * L0B_PP_SIZE);
