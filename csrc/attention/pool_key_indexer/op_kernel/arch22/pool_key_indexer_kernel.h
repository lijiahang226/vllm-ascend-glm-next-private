...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *poolKey, __gm__ uint8_t *weights,
                                __gm__ uint8_t *poolTailK, __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                __gm__ uint8_t *blockTable, __gm__ uint8_t *qDescale, __gm__ uint8_t *kDescale, __gm__ uint8_t *sparse_indices, __gm__ uint8_t *sparse_values,
...
template <typename LIT>
__aicore__ inline void PoolKeyIndexerKernel<LIT>::GetS1S2ActualSeqLen(uint32_t bIdx,
                                                                      uint32_t &actS1Size, uint32_t &actS2Size, uint32_t &actS2SizeOrig)
{
...
__aicore__ inline uint32_t PoolKeyIndexerKernel<LIT>::GetTotalBaseBlockNum()
{
    ...
    for (uint32_t bIdx = 0; bIdx < constInfo.batchSize; bIdx++) {
        ...
        for (uint32_t s1gIdx = 0; s1gIdx < s1GBaseNum; s1gIdx++) {
            s2BaseNum = constInfo.isSparseCountOver2K ? (actS2Size > 0 ? 1 : 0) : GetS2BaseBlockNumOnMask(s1gIdx, actS1Size, actS2SizeOrig);
...
// 多核版本，双闭区间
template <typename LIT>
__aicore__ void inline PoolKeyIndexerKernel<LIT>::SplitCore(uint32_t curCoreIdx,
                                                            uint32_t &coreNum, PkiCommon::SplitCoreInfo &info)
{
...
__aicore__ inline void PoolKeyIndexerKernel<LIT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx, uint32_t s1Start)
{
    if ASCEND_IS_AIV {
        ...
        uint32_t idxOutStride = (constInfo.poolSize > 1) ? (constInfo.sparseCount * constInfo.poolSize + constInfo.poolSize - 1) : constInfo.sparseCount;
...
__aicore__ inline void PoolKeyIndexerKernel<LIT>::Init(__gm__ uint8_t *query,
                                                       ...
                                                       __gm__ uint8_t *poolTailK, __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                                       __gm__ uint8_t *blockTable, __gm__ uint8_t *qDescale, __gm__ uint8_t *kDescale, __gm__ uint8_t *sparse_indices,
                                                       __gm__ uint8_t *sparse_values,
                                                              ...
                                                              uint32_t s2LoopIdx, PkiCommon::RunInfo &runInfo)
{
    ...
    if (runInfo.isFirstS2InnerLoop) {
        ...
        if constexpr (LAYOUT_T == PkiLayout::TND) {
            ...
            actualSeqKPrefixSum = (runInfo.bIdx <= 0) ? 0 : static_cast<uint64_t>(actualSeqLengthsGm.GetValue(runInfo.bIdx - 1));
        } else { // BSND
        ...
        uint32_t idxOutStride = (constInfo.poolSize > 1) ? (constInfo.sparseCount * constInfo.poolSize + constInfo.poolSize - 1) : constInfo.sparseCount;
        indiceOutCoreOffset = actualSeqQPrefixSum * constInfo.kHeadNum * idxOutStride +
                              runInfo.n2Idx * idxOutStride;
        ...
        valueOutCoreOffset = actualSeqQPrefixSum * constInfo.kHeadNum * constInfo.sparseCount +
        ...
        if (constInfo.returnValue) {
            ...
            if (valueBase < totalValueSize) {
                uint64_t valueDealSize =
                    (valueBase + singleCoreValueSize <= totalValueSize) ? singleCoreValueSize : totalValueSize - valueBase;
