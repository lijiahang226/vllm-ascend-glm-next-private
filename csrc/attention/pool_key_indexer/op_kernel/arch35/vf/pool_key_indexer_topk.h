...
public:
    ...
    __aicore__ inline void operator()(LocalTensor<uint32_t>& mrgValueLocal, LocalTensor<uint32_t>& indicesOutLocal,
                                      ...
                                      uint32_t loopIdx, uint32_t s2LoopNum, bool returnValue)
    {
        if (s2LoopNum == 1) {
            if (returnValue) {
            ...
            PipeBarrier<PIPE_V>();
            AscendC::DataCopy(indicesOutLocal, tmpIndexLocal, PkiCommon::Align(topK, (uint32_t)256)); // 256: 本地内存对齐基线
        } else {
            if (loopIdx == 0) {
                ...
                uint32_t loopBasicIdx = topK < trunkLen ?
                         (loopIdx * trunkLen - PkiCommon::Align(topK, (uint32_t)256)) :
                         ((loopIdx - 1) * trunkLen);
